...
namespace EstoqueMercado
{
    public partial class TelaCadastro : Form
    {
        ...
        public TelaCadastro(TelaInicial formInicial, int indice = -1)
        {
            ...
            if (indice >= 0)
            {
                ProdutoAtual = DadosSingleton.Instancia().ObterProduto(indice);
                CarregarCampos();
            }
            else
            {
        ...
        private void AoClicarEmSalvar(object sender, EventArgs e)
        {
            ...
            if (Indice >= 0)
            {
                DadosSingleton.Instancia().AtualizarProduto(Indice, ProdutoAtual);
            }
            else
            {
                DadosSingleton.Instancia().AdicionarProduto(ProdutoAtual);
            }
